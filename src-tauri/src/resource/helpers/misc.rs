...
pub fn get_download_api(source: SourceType, resource_type: ResourceType) -> Result<Url, ResourceError> {
    match source {
        SourceType::Official => match resource_type {
            ResourceType::VersionManifest => Ok(Url::parse("https://launchermeta.mojang.com/mc/game/version_manifest.json").expect("static URL")),
            ResourceType::ForgeMeta => Err(ResourceError::NoDownloadApi),
            ResourceType::OptiFine => Err(ResourceError::NoDownloadApi),
            ResourceType::FabricMeta => Ok(Url::parse("https://meta.fabricmc.net/").expect("static URL")),
            ResourceType::NeoforgeMetaForge => Ok(Url::parse("https://maven.neoforged.net/api/maven/versions/releases/net/neoforged/forge/").expect("static URL")),
            ResourceType::NeoforgeMetaNeoforge => Ok(Url::parse("https://maven.neoforged.net/api/maven/versions/releases/net/neoforged/neoforge/").expect("static URL")),
            ResourceType::QuiltMeta => Ok(Url::parse("https://meta.quiltmc.org/").expect("static URL")),
        },
        SourceType::BMCLAPIMirror => match resource_type {
            ResourceType::VersionManifest => Ok(Url::parse("https://bmclapi2.bangbang93.com/mc/game/version_manifest.json").expect("static URL")),
            ResourceType::ForgeMeta => Ok(Url::parse("https://bmclapi2.bangbang93.com/forge/").expect("static URL")),
            ResourceType::FabricMeta => Ok(Url::parse("https://bmclapi2.bangbang93.com/fabric-meta/").expect("static URL")),
            ResourceType::NeoforgeMetaForge | ResourceType::NeoforgeMetaNeoforge => Ok(Url::parse("https://bmclapi2.bangbang93.com/neoforge/").expect("static URL")),
            ResourceType::OptiFine => Ok(Url::parse("https://bmclapi2.bangbang93.com/optifine/").expect("static URL")),
            ResourceType::QuiltMeta => Ok(Url::parse("https://bmclapi2.bangbang93.com/quilt-meta/").expect("static URL")),
        },
    }
}
...
pub fn version_pack_sort(a: &OtherResourceVersionPack, b: &OtherResourceVersionPack) -> Ordering {
    ...
    fn compare_versions_with_suffix(
        v1: &[u32], suffix1: &str, v2: &[u32], suffix2: &str,
    ) -> Ordering {
...
pub async fn apply_other_resource_enhancements(
    ...
    let translated_desc = match resource_info.source {
        ...
        OtherResourceSource::CurseForge => translate_description_curseforge(app, &resource_info.id).await,
        _ => Ok(None),
    };
