...
fn calculate_similarity(source: &str, query: &str) -> f64 {
    ...
    while qp < query_chars.len() {
        ...
        if len_max > 0 {
            ...
            let position_bonus =
                1.0 + 0.3 * (3.0 - (qp as i32 - sp_max as i32).abs() as f64).max(0.0);
...
impl ModDataBase {
    ...
    pub fn get_translated_name(
        ...
        self.get_mod_record(resource_slug, source)
...
pub async fn handle_search_query(app: &AppHandle, query: &str) -> Result<String, ResourceError> {
    ...
    let search_results = {
        ...
        cache.get_mods_by_chinese(&query, 5)
    };
    ...
    let mut keyword_scores: Vec<(String, f64)> = keyword_count
        ...
        .map(|(keyword, count)| {
            ...
            let length_bonus = (keyword.len() as f64 / 10.0).min(1.0) + 1.0;
